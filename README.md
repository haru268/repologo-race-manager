...
**注意**: 
- Firebaseを設定しない場合、アプリはローカルストレージのみを使用して動作します（既存機能は維持されます）
- **Vercelなどのホスティングサービスを使用している場合、環境変数をホスティングサービスのダッシュボードで設定する必要があります**
  - 詳細は `FIREBASE_SETUP.md` を参照してください
- Firebase設定が正しくない場合、ヘッダーに「📦 ローカルのみ」と表示されます
- ブラウザのコンソール（F12キー）でエラーメッセージを確認できます
